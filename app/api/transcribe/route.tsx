  ...
  try {
    ...
    if (!audioFile) {
      return NextResponse.json({
        transcript: "I have symptoms I'd like to discuss.",
        fallback: true,
      });
    }
    ...
    // Poll for completion (internal to API)
    let attempts = 0;
    while (attempts < 20) {
      attempts++;
      ...
      if (data.status === "completed") {
        console.log("✅ Transcription done:", data.text);
        return NextResponse.json({
...
//     if (!apiKey) {
//       console.log("❌ AssemblyAI API key missing");
//       return NextResponse.json({
//         transcript: "I have symptoms I'd like to discuss.",
//         fallback: true,
//       });
...
//       console.log(`🔍 Poll ${attempts}/${maxAttempts}: ${status}`);
...
//     return NextResponse.json({ transcript, fallback: false });
//   } catch (error: any) {
...
// // Add GET method handler (even though not used, prevents 405 error)
// export async function GET(req: NextRequest) {
//   return NextResponse.json(
//     { error: "Use POST method to transcribe audio" },
//     { status: 405 }
//   );
// }
