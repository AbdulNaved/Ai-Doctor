  ...
  try {
    ...
    try {
      ...
      if (doctorId && doctorIdToVoiceMapping[doctorId]) {
        ...
        console.log(
          `Using voice mapping for doctorId ${doctorId}: ${murfVoiceId}`
        );
      } else if (voiceId && voiceIdMapping[voiceId]) {
        murfVoiceId = voiceIdMapping[voiceId];
        console.log(
          `Using voice mapping for voiceId ${voiceId}: ${murfVoiceId}`
        );
      }

      console.log(
        `Calling Murf AI API with voice: ${murfVoiceId}, text length: ${text.length} chars`
      );
      ...
      if (response.data && response.data.audioFile) {
        console.log(
          `Murf AI API returned audio URL: ${response.data.audioFile.substring(
            0,
            50
          )}...`
        );
        ...
        console.log(
          `Successfully fetched audio data from Murf AI, size: ${audioResponse.data.byteLength} bytes`
        );
      ...
      console.error(
        "Error details:",
        murfError.response?.data || "No response data"
      );
